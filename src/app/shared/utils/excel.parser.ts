...
export const convertExcelRowToBasicTenant = (row: Row): any => {
  return <any>{
    name: `${row[0]}`,
    insideQueueCapacity: `${row[1]}`,
    initialInsideQueue: `${row[2]}`,
    hourlyProcessingSpeedMax: `${row[3]}`,
    parameterTenantParameterAccessPoints: String(row[4]).split(','),
    gateToTenantTimeInMin: `${row[5]}`,
  }
}
...
export const generateBasicExcel = async (lang: string): Promise<Blob> => {
  ...
  list.getColumn(4).width = 25;
  list.getColumn(5).width = 25;
  ...
  const data = [
    lang === 'en' ?
      {
        'Tenant': '',
        'Inside queue capacity': '',
        'Initial inside queue': '',
        'Hourly processing speed (maximum)': '',
        'Designated access point': '',
        'Gate-to-tenant time (min)': '',
      } : {
        'Tenant': '',
        'Inside queue capacity': '',
        'Initial inside queue': '',
...
export const generateTenantExcel = async (lang: string): Promise<Blob> => {
  const workbook = new ExcelJS.Workbook();
  const list = workbook.addWorksheet('basic');
  list.getColumn(1).width = 15;
  list.getColumn(2).width = 25;
...
export const generateBookingExcel = async (lang: string): Promise<Blob> => {
  const workbook = new ExcelJS.Workbook();
  const list = workbook.addWorksheet('basic');
  list.getColumn(1).width = 25;
  list.getColumn(2).width = 15;
  list.getColumn(3).width = 15;
  list.getColumn(4).width = 15;
  list.getColumn(5).width = 15;
  list.getColumn(6).width = 15;
